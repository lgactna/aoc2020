...
def gold_bag_finder(bag_type):
    ...
    returns [last-level, count]
    count is the total number of base containing bags
    '''
    total_bag_count = 0
    ...
    for rule in rules:
        #if dead end, don't try additional recursion - this is the end, go to the next rule
        #but also add the dead end bags themselves
        if not bag_dict[rule[1]]:
            print(f"{rule[1]} - {bag_dict[rule[1]]} (dead end)")
            total_bag_count += rule[0]
            continue
        else:
            #account for the larger bags themselves
            total_bag_count += rule[0]
            
            #multiply by the number of smaller bags (and their children) by the number of bags
            #for this rule
            output = gold_bag_finder(rule[1])
            print(f"multiplying {rule[0]} by {output}, for {rule[1]}")
            total_bag_count += rule[0]*gold_bag_finder(rule[1])

            #the net result here is that if we have two orange bags containing two yellow bags each
            #we count the two orange bags
            #and then the four yellow bags inside the two orange bags
            #do this for each child rule
