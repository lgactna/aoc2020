...
def validator(passport_dict):
    ...
    if int(passport_dict["eyr"]) < 2020 or int(passport_dict["eyr"]) > 2030:
        return False
    height_type = passport_dict["hgt"][-2:]
    if height_type == "cm":
...
for passport in passports:
    #check if should even bother
    fields_present = [a[-3:] for a in passport.split(":")]
    del fields_present[-1]
    if len(fields_present) == 8 or (len(fields_present) == 7 and ("cid" not in fields_present)):
        #split into fields and work from there
        passport = passport.replace("\n", " ")
        fields = passport.split(" ")
        fields = [a.split(":") for a in fields]
        field_dict = {}
        for field in fields:
            field_dict[field[0]] = field[1]
        try:
            if validator(field_dict):
                valid += 1
        except:
            continue
